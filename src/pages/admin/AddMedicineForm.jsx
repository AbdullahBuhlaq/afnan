...
function AddMedicineForm(props) {
  ...
  async function addMedicine() {
    try {
      ...
      setDuringAdd(true);
      const response = await fetch("http://localhost:3001/medicine/create", infoRequestOptions);
      const data = await response.json();
      if (data.success) {
        props.setMedicines({ ...props.medicines, [data.data]: { ...newData, id: data.data } });
  ...
  try {
    return (
      <>
        ...
        <Button action={addMedicine} text={"Add"} disabled={duringAdd} joiObject={joiMedicine} state={medicine} setStateErrors={setMedicineErrors} toast={props.toast} />
      </>
    );
  } catch (err) {
    console.log(err);
